...
- Scouts may target B2B, B2B2C, or B2C; state who pays and who uses. No motion is preferred or excluded by default, and the evidence bar is the same for all three. B2C entries must address churn and customer acquisition cost explicitly rather than assuming them away.

## Change log

- 2026-07-30 — Claude (Cowork): Opened business model scope to B2B, B2B2C, and B2C at the repo owner's direction. A concurrent edit by another agent made the same change; this merge keeps that agent's wording as the base and adds only the B2C churn and CAC requirement. No journal entries were edited and no other rule changed.
